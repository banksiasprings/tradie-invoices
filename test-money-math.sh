#!/usr/bin/env bash
# test-money-math.sh — run the money-path regression tests against the app
# running in the Pixel_7_API34 emulator (via Chrome DevTools Protocol).
#
# Prereq: emulator up with the app installed + launched (bash test-emulator.sh).
# Usage:  bash test-money-math.sh
set -uo pipefail
...
# `ws` module: use a global install. Override WS_NODE_PATH if yours differs.
...
echo "── Money-path tests (live app via CDP) ──"
NODE_PATH="$NODE_PATH_DIR" node "$HERE/test-money-math.js" "$WS"
RC=$?
"$ADB" -s "$SERIAL" forward --remove tcp:$PORT >/dev/null 2>&1 || true
exit $RC
