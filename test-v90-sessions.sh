#!/usr/bin/env bash
# test-v90-sessions.sh — run the v90 multi-session queue tests against the app
# running in the emulator (via Chrome DevTools Protocol).
#
# Prereq: emulator up with the v90 app installed + launched.
# Usage:  bash test-v90-sessions.sh
set -uo pipefail
...
echo "── v90 multi-session queue tests (live app via CDP) ──"
NODE_PATH="$NODE_PATH_DIR" node "$HERE/test-v90-sessions.js" "$WS"
RC=$?
"$ADB" -s "$SERIAL" forward --remove tcp:$PORT >/dev/null 2>&1 || true
exit $RC
